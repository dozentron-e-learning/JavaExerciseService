...
RSpec.describe ValidateSubmissionJob, type: :job do
  ...
  let(:submission_download_url) { "#{SUBMISSION_SERVICE_URL}/submissions/#{submission_id}/download" }
  let(:submission_update_url) { "#{SUBMISSION_SERVICE_URL}/submissions/#{submission_id}" }
